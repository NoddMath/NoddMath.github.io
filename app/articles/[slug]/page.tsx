...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="article-page">
      <header className="reader-header"><a className="brand" href="/"><i className="brand-mark" aria-hidden="true" /><span>微光与方程</span></a><a className="back-link" href="/">← 返回观测站</a></header>
      <main className="reader-shell">
        <div className="article-kicker">{post.categories.join(" · ")} <span>/</span> {post.date}</div>
        <h1>{post.title}</h1>
        <p className="article-deck">{post.excerpt}</p>
        <div className="article-details"><span>{post.readingTime}</span><span>Markdown · LaTeX</span></div>
